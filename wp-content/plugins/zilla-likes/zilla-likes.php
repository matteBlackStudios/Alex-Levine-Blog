...
class ZillaLikes {
	...
	function like_this($post_id, $zero_postfix = false, $one_postfix = false, $more_postfix = false, $action = 'get') 
	{
		...
		switch($action) {
			...
			case 'get':
				...
				return '<div class="title like"><i class="fa fa-heart" aria-hidden="true"></i> <span>Like</span> <span class="zilla-likes-count count">'. $likes .'</span> <span class="zilla-likes-postfix">'. $postfix .'</span></div>';
				break;
			...
			case 'update':
				...
				if( isset($_COOKIE['zilla_likes_'. $post_id]) ){
					...
					$likes++;
					update_post_meta($post_id, '_zilla_likes', $likes);
					setcookie('zilla_likes_'. $post_id, $post_id,  time() + 60 * 60 * 24 * 30, '/');
				}
	...
	function do_likes()
	{
		...
		if(isset($_COOKIE['zilla_likes_'. get_the_ID()]) === true){
	...
	function widget( $args, $instance ) {
		...
		foreach( $likes_posts as $likes_post ) {
			...
			if( $display_count ) {
				...
				$count_output = "<div class='title like'><i class='fa fa-heart' aria-hidden='true'></i> <span>Like</span> <span class='zilla-likes-count count'>($count)</span></div>";
			}
